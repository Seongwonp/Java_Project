package ch_07_method;

import java.util.Scanner;

public class My02_01 {
    static final int People_Num = 3;//인원 수
    static Scanner scanner = new Scanner(System.in);

    //사용자로부터 데이터 입력
    static void inputData(int[] height, int[] weight, int[] age){
        for(int i=0; i<People_Num; i++){
            System.out.print("[" + (i+1) + "] ");
            System.out.print("신장: ");
            height[i] = scanner.nextInt();
            System.out.print("    체중: ");
            weight[i] = scanner.nextInt();
            System.out.print("    나이: ");
            age[i] = scanner.nextInt();
        }
    }

    //신장,체중,나이의 최대값 구하기
    static int getMax(int[] arr){
        int max =arr[0];
        for(int i=1; i<People_Num; i++){
            if(arr[i] > max) {
                max = arr[i];
            }
        }
        return max;
    }

    //신장,체중,나이의 최소값 구하기
    static int getMin(int[] arr){
        int min =arr[0];
        for(int i=1; i<People_Num; i++){
            if(arr[i] < min) {
                min = arr[i];
            }
        }
        return min;
    }

    public static void main(String[] args) {
        /*
            메서드는 왜 필요한가?
            입력받은  3명의 신장, 체중, 나이의 최소값을 구해서 출력
         */
        Scanner scanner = new Scanner(System.in);
        int[] height = new int[People_Num];
        int[] weight = new int[People_Num];
        int[] age    = new int[People_Num];

        //사용자로부터 데이터 입력
        inputData(height, weight, age);

        //신장,체중,나이 배열주소를 메소드에 대입
        int minHeight = getMin(height);
        int minWeight = getMin(weight);
        int minAge = getMin(age);

        System.out.println("신장의 최소값은 " + minHeight + "입니다.");
        System.out.println("체중의 최소값은 " + minWeight + "입니다.");
        System.out.println("나이의 최소값은 " + minAge + "입니다.");
        scanner.close();
    }
}
